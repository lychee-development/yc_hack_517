...
class Person:
    def __init__(self, sys_prompt, model="claude-3-5-haiku-latest", memory_model = "claude-3-5-haiku-latest", temp=np.random.normal(), max_tokens=2048):
        self.id = uuid.uuid4()  # Changed from uuid5() which requires namespace
        self.sys_prompt = sys_prompt # TODO if we can, we should probably retrieve this via the MCP server, as mcp prompts would be perfect for this use case.
        self.temp = temp
        self.model = model
    ...
    async def call_llm(self, mcp_session):
